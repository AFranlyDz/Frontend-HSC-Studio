...
const PlanillaRecoleccion = ({ paciente }) => {
  // Función para validar y obtener valor seguro
  ...
  // Función para formatear booleanos de manera segura
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.patientInfo}>
            Paciente: {safeValue(paciente.seudonimo)} ({safeValue(paciente.nombre)} {safeValue(paciente.apellidos)})
          </Text>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.row}>
            ...
            <Text style={styles.value}>{safeValue(paciente.numero)}</Text>
          </View>

          <View style={styles.row}>
            <Text style={styles.label}>Nombre completo:</Text>
            <Text style={styles.value}>
              {safeValue(paciente.nombre)} {safeValue(paciente.apellidos)}
            </Text>
          </View>

          <View style={styles.row}>
            <Text style={styles.label}>Seudónimo:</Text>
            <Text style={styles.value}>{safeValue(paciente.seudonimo)}</Text>
          </View>

          <View style={styles.row}>
            <Text style={styles.label}>Edad:</Text>
            <Text style={styles.value}>{safeValue(paciente.edad)}</Text>
          </View>
        ...
        {paciente.rcg && Array.isArray(paciente.rcg) && paciente.rcg.length > 0 && (
          <View style={styles.section}>
            ...
            {paciente.rcg.map((factor) => (
              <View key={factor.id || Math.random()} style={styles.tableRow}>
                <Text style={styles.tableCellSmall}>{safeValue(factor.id)}</Text>
                <Text style={styles.tableCell}>{safeValue(factor.codificador?.nombre)}</Text>
                <Text style={styles.tableCell}>{safeValue(factor.codificador?.nombre_corto)}</Text>
                <Text style={styles.tableCell}>{safeValue(factor.codificador?.descripcion)}</Text>
                <Text style={styles.tableCell}>{safeValue(factor.codificador?.clasificacion)}</Text>
                <Text style={styles.tableCell}>{safeValue(factor.notas)}</Text>
              </View>
            ))}
          </View>
        )}
        ...
        {/* Episodios */}
        {paciente.episodios && Array.isArray(paciente.episodios) && paciente.episodios.length > 0 && (
          <View style={styles.section}>
            <Text style={styles.subtitle}>EPISODIOS CLÍNICOS</Text>

            {paciente.episodios.map((episodio) => (
              <View key={episodio.id || Math.random()} style={{ marginBottom: 20 }}>
                <Text style={styles.sectionTitle}>
                  EPISODIO {safeValue(episodio.id)} - {formatDate(episodio.inicio)} a {formatDate(episodio.fecha_alta)}
                </Text>
                ...
                <View style={styles.subsection}>
                  ...
                  <View style={styles.row}>
                    ...
                    <Text style={styles.value}>{safeValue(episodio.tiempo_estadia)} días</Text>
                  </View>
                  ...
                  <View style={styles.row}>
                    ...
                    <Text style={styles.value}>{safeValue(episodio.tiempo_antecedente)}</Text>
                  </View>

                  <View style={styles.row}>
                    <Text style={styles.label}>Descripción antecedente:</Text>
                    <Text style={styles.value}>{safeValue(episodio.descripcion_antecedente)}</Text>
                  </View>

                  <View style={styles.row}>
                    <Text style={styles.label}>Edad del paciente:</Text>
                    <Text style={styles.value}>{safeValue(episodio.edad_paciente)}</Text>
                  </View>

                  <View style={styles.row}>
                    <Text style={styles.label}>Observaciones:</Text>
                    <Text style={styles.value}>{safeValue(episodio.observaciones)}</Text>
                  </View>
                </View>

                {/* Rasgos clínicos del episodio (RCE) */}
                {episodio.rce && Array.isArray(episodio.rce) && episodio.rce.length > 0 && (
                  <View style={styles.subsection}>
                    ...
                    {episodio.rce.map((rce) => (
                      <View key={rce.id || Math.random()} style={styles.tableRow}>
                        <Text style={styles.tableCellSmall}>{safeValue(rce.id)}</Text>
                        <Text style={styles.tableCell}>{safeValue(rce.codificador?.nombre)}</Text>
                        <Text style={styles.tableCell}>{safeValue(rce.codificador?.nombre_corto)}</Text>
                        <Text style={styles.tableCell}>{safeValue(rce.codificador?.descripcion)}</Text>
                        <Text style={styles.tableCell}>{safeValue(rce.codificador?.clasificacion)}</Text>
                        <Text style={styles.tableCell}>{safeValue(rce.tiempo)}</Text>
                        <Text style={styles.tableCell}>{safeValue(rce.notas)}</Text>
                      </View>
                    ))}
                  </View>
                )}
                ...
                {episodio.hematomas_subdurales &&
                  Array.isArray(episodio.hematomas_subdurales) &&
                  episodio.hematomas_subdurales.length > 0 && (
                    <View style={styles.subsection}>
                      ...
                      {episodio.hematomas_subdurales.map((hematoma) => (
                        <View key={hematoma.id || Math.random()} style={{ marginBottom: 10 }}>
                          <View style={styles.row}>
                            <Text style={styles.label}>ID Hematoma:</Text>
                            <Text style={styles.value}>{safeValue(hematoma.id)}</Text>
                          </View>
                          ...
                          <View style={styles.row}>
                            ...
                            <Text style={styles.value}>{safeValue(hematoma.localización)}</Text>
                          </View>

                          <View style={styles.row}>
                            <Text style={styles.label}>Topografía:</Text>
                            <Text style={styles.value}>{safeValue(hematoma.topografia)}</Text>
                          </View>
                          ...
                          <View style={styles.row}>
                            <Text style={styles.label}>Observaciones:</Text>
                            <Text style={styles.value}>{safeValue(hematoma.observaciones)}</Text>
                          </View>
                        </View>
                      ))}
                    </View>
                ...
                {episodio.registro_operatorio &&
                  Array.isArray(episodio.registro_operatorio) &&
                  episodio.registro_operatorio.length > 0 && (
                    <View style={styles.subsection}>
                      ...
                      {episodio.registro_operatorio.map((registro) => (
                        <View key={registro.id || Math.random()} style={{ marginBottom: 15 }}>
                          <View style={styles.row}>
                            <Text style={styles.label}>ID Registro:</Text>
                            <Text style={styles.value}>{safeValue(registro.id)}</Text>
                          </View>
                          ...
                          <View style={styles.row}>
                            <Text style={styles.label}>Observaciones:</Text>
                            <Text style={styles.value}>{safeValue(registro.observaciones)}</Text>
                          </View>

                          {/* Registros posoperatorios */}
                          {registro.registros_posoperatorios &&
                            Array.isArray(registro.registros_posoperatorios) &&
                            registro.registros_posoperatorios.length > 0 && (
                              <View style={{ marginTop: 10 }}>
                                ...
                                {registro.registros_posoperatorios.map((posop) => (
                                  <View key={posop.id || Math.random()} style={styles.tableRow}>
                                    <Text style={styles.tableCellSmall}>{safeValue(posop.id)}</Text>
                          ...
                          {registro.rasgos_clinicos_operatorios &&
                            Array.isArray(registro.rasgos_clinicos_operatorios) &&
                            registro.rasgos_clinicos_operatorios.length > 0 && (
                              <View style={{ marginTop: 10 }}>
                                ...
                                {registro.rasgos_clinicos_operatorios.map((rasgo) => (
                                  <View key={rasgo.id || Math.random()} style={styles.tableRow}>
                                    <Text style={styles.tableCellSmall}>{safeValue(rasgo.id)}</Text>
                                    <Text style={styles.tableCell}>{safeValue(rasgo.codificador?.nombre)}</Text>
                                    <Text style={styles.tableCell}>{safeValue(rasgo.codificador?.nombre_corto)}</Text>
                                    <Text style={styles.tableCell}>{safeValue(rasgo.codificador?.descripcion)}</Text>
                                    <Text style={styles.tableCell}>{safeValue(rasgo.codificador?.clasificacion)}</Text>
                                  </View>
                                ))}
                              </View>
                            )}
