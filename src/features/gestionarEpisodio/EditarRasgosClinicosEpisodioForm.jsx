...
export const EditarRasgosClinicosEpisodioForm = ({ episodio, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
          ...
          if (asociacionId) {
            promesas.push(
              axios
                ...
                .then(() => console.log(`Eliminado rasgo clínico de episodio con ID ${asociacionId}`))
                .catch((error) => {
                  console.error(`Error al eliminar rasgo clínico de episodio con ID ${asociacionId}:`, error)
                  throw error
                }),
            )
          }
        ...
        if (rasgosSeleccionados[codificadorId] && !estadoInicial.rasgos[codificadorId]) {
          ...
          promesas.push(
            axios
              ...
              .then((response) => console.log(`Añadido nuevo rasgo clínico de episodio:`, response.data))
              .catch((error) => {
                console.error(`Error al añadir nuevo rasgo clínico de episodio:`, error)
                throw error
              }),
          )
        }
      ...
      sessionStorage.setItem("selectedPaciente", JSON.stringify(response.data))

      // Actualizar el episodio seleccionado en sessionStorage
      const episodioActualizado = response.data.episodios.find((ep) => ep.id === episodio.id)
      if (episodioActualizado) {
        sessionStorage.setItem("selectedEpisodio", JSON.stringify(episodioActualizado))
      }
      ...
      console.error("Error al actualizar los rasgos clínicos del episodio:", error)
      alert("Error al actualizar los rasgos clínicos del episodio. Por favor, intente nuevamente.")
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-semibold text-gray-800">Editar Rasgos Clínicos del Episodio</h3>
        <button
          type="submit"
                      ...
                      <div className="mt-2 space-y-3">
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => handleTiempoChange(codificador.id, e.target.value)}
                            min="0"
